...
def test_send_overlapping_audit_log(run_count, user, fixed_datetime, parallel_session_setup, db_no_flush):  
    addresses = []
...
def test_send_overlapping_audit_log__use_django_auditlog(run_count, user, fixed_datetime, parallel_session_setup, db_no_flush):
    addresses = []
    for _ in range(AUDIT_LOG_COUNT):
        addresses.append(generate_random_ip())
